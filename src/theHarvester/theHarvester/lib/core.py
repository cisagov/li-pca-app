    ...
    def banner() -> None:
        ...
        print(
            "* theHarvester {version}{filler}*".format(
                version=Core.version(), filler=" " * (51 - len(Core.version()))
            )
        )
...
class AsyncFetcher:
    ...
    async def post_fetch(
        cls, url, headers="", data="", params="", json=False, proxy=False
    ):
        if len(headers) == 0:
    ...
    @staticmethod
    async def fetch(
        session, url, params="", json=False, proxy=""
    ...
    @classmethod
    async def fetch_all(
        cls, urls, headers="", params="", json=False, takeover=False, proxy=False
    ) -> list:
