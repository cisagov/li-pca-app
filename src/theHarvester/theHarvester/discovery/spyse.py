...
from spyse import Client, DomainSearchParams, Operators, QueryParam, SearchQuery
...
class SearchSpyse:
    ...
    async def do_search(self):
        ...
        query = SearchQuery()
        query.append_param(
            QueryParam(DomainSearchParams.name, Operators.ends_with, self.word)
        )
        ...
        try:
            ...
            if (
                total > self.client.SEARCH_RESULTS_LIMIT
                and self.client.account.is_scroll_search_enabled
            ):
                scroll_id = None
                while True:
                ...
                while True:
                    limit = (
                        max_limit
                        if self.limit - fetch_count > max_limit
                        else self.limit - fetch_count
                    )
                    if limit <= 0:
                        break
