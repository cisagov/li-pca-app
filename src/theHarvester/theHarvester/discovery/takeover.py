    ...
    async def check(self, url, resp):
        ...
        for match in matches:
            ...
            if match in self.fingerprints.keys():
                ...
                print(
                    f"\t\033[91m Type of takeover is: {self.fingerprints[match]}\033[1;32;40m"
                )

    async def do_take(self):
        try:
            if len(self.hosts) > 0:
                tup_resps: list = await AsyncFetcher.fetch_all(
                    self.hosts, takeover=True, proxy=self.proxy
                )
